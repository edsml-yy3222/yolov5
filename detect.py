...
def run(
    ...
    name="default",  # save results to project/name
    ...
    (save_dir / "detections" if save_txt else save_dir).mkdir(
        parents=True, exist_ok=True
    )  # make dir

    img_save_dir = save_dir / "images"
    isExist = os.path.exists(img_save_dir)
    if not isExist:
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            save_path = str(img_save_dir / p.name)  # im.jpg
            txt_path = str(save_dir / "detections" / p.stem) + (
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    if save_txt:  # Write to file
                        ...
                        line = line[1:]
                        # line.insert(0, 0)
                        with open(f"{txt_path}.csv", "a", newline="") as f:
                            writer = csv.writer(f)
    ...
    if save_txt or save_img:
        s = (
            f"\n{len(list(save_dir.glob('labels/*.csv')))} labels saved to {save_dir / 'detections'}"
            if save_txt
            else ""
        )
